...
static constexpr int32_t NUM_ELEMENTS = 1000000;
static constexpr int32_t NUM_UPDATES = 1000;
std::mt19937 mt_rand;

class SortedInsertFixture : public benchmark::Fixture {
 public:
  void SetUp(const ::benchmark::State&) final {
    std::vector<int32_t> unique_ids(NUM_ELEMENTS + NUM_UPDATES);
    std::iota(unique_ids.begin(), unique_ids.end(), 0);
    std::shuffle(unique_ids.begin(), unique_ids.end(), mt_rand);

    std::vector<int32_t> values(unique_ids.begin(), unique_ids.begin() + NUM_ELEMENTS);
    ...
    std::transform(
        ...
        [](const int32_t key, const int32_t value) { return std::make_pair(key, value); });
  }

  void TearDown(const ::benchmark::State&) final {}

  std::vector<int32_t> sorted_values_;
  std::unordered_map<int32_t, int32_t> updates_;
};

BENCHMARK_DEFINE_F(SortedInsertFixture, SetInsertion)(benchmark::State& state) {
  std::set<int32_t> s(sorted_values_.begin(), sorted_values_.end());
  for (auto _ : state) {
